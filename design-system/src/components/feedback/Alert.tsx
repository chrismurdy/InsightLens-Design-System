...
const TONES: Record<NonNullable<AlertProps["tone"]>, { border: string; bg: string; fg: string; icon: string }> = {
...
/** Inline status banner for platform screens. Intentional addition — see readme.md. */
export function Alert({ tone = "info", title, children, onDismiss, style }: AlertProps) {
  ...
  return (
    <div role="status" style={{
      ...
      <div style={{ flex: 1 }}>
        {title ? <div className="il-small" style={{ fontWeight: "var(--il-weight-semibold)" as React.CSSProperties["fontWeight"], color: "var(--il-ink-900)" }}>{title}</div> : null}
